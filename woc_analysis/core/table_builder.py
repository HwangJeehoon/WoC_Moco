...
def _extract_run_prefix_and_number(id_series: pd.Series) -> tuple[pd.Series, pd.Series]:
    """Extract alphabetic prefix and numeric suffix from run IDs.

    Examples:
        sp001 -> run_prefix=sp, run_number=1
        af006 -> run_prefix=af, run_number=6
    """
    text = id_series.fillna("").astype(str).str.strip()
    extracted = text.str.extract(r"^([A-Za-z]+)\s*0*([0-9]+)")
    run_prefix = extracted[0].fillna("").str.lower()
    run_number = pd.to_numeric(extracted[1], errors="coerce").astype("Int64")
    return run_prefix, run_number
...
def add_run_labels(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Prefix/number columns are used for filtering and plot grouping.
    out["run_prefix"], out["run_number"] = _extract_run_prefix_and_number(id_series)

    # Existing assist label logic: derive the assist code from the second
    # alphabetic character of the run ID prefix when available.
    prefix_upper = out["run_prefix"].astype(str).str.upper()
    assist_code = prefix_upper.str[1].fillna("")
...
def build_master_table(queue_file: str | Path, metric_file: str | Path) -> pd.DataFrame:
    ...
    text_columns = {
        ...
        "assist_label", "opt_mode_label", "run_prefix",
    }
...
def get_categorical_columns(df: pd.DataFrame) -> list[str]:
    preferred_first = [
        "run_prefix", "health_label", "symmetry_label", "assist_label", "opt_mode_label", "model",
